...
// versions:
//   sqlc v1.31.1
// source: merchant_code_audits.sql
...
const createMerchantCodeAudit = `-- name: CreateMerchantCodeAudit :one
INSERT INTO referral_code_bonus.merchant_code_audits
    (merchant_id, result, checked_urls, matched, note, deactivated)
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, merchant_id, result, checked_urls, matched, note, deactivated, created_at
`

type CreateMerchantCodeAuditParams struct {
...
func (q *Queries) CreateMerchantCodeAudit(ctx context.Context, arg CreateMerchantCodeAuditParams) (MerchantCodeAudit, error) {
	row := q.db.QueryRow(ctx, createMerchantCodeAudit,
		arg.MerchantID,
		arg.Result,
		arg.CheckedUrls,
		arg.Matched,
...
const deactivateMerchantForCodeAudit = `-- name: DeactivateMerchantForCodeAudit :execrows
...
func (q *Queries) DeactivateMerchantForCodeAudit(ctx context.Context, id uuid.UUID) (int64, error) {
	result, err := q.db.Exec(ctx, deactivateMerchantForCodeAudit, id)
	if err != nil {
		return 0, err
	}
...
const listMerchantsDueForCodeAudit = `-- name: ListMerchantsDueForCodeAudit :many
SELECT
...
WHERE last.created_at IS NULL OR last.created_at < now() - make_interval(days => $1::int)
ORDER BY last.created_at ASC NULLS FIRST, m.name
`

type ListMerchantsDueForCodeAuditRow struct {
...
// 服務商最可能根本沒在發碼，先問它們。
func (q *Queries) ListMerchantsDueForCodeAudit(ctx context.Context, staleDays int32) ([]ListMerchantsDueForCodeAuditRow, error) {
	rows, err := q.db.Query(ctx, listMerchantsDueForCodeAudit, staleDays)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMerchantsDueForCodeAuditRow{}
	for rows.Next() {
		var i ListMerchantsDueForCodeAuditRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.SignupUrl,
...
const listMerchantsFlaggedByCodeAudit = `-- name: ListMerchantsFlaggedByCodeAudit :many
WITH recent AS (
    SELECT
...
type ListMerchantsFlaggedByCodeAuditParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

type ListMerchantsFlaggedByCodeAuditRow struct {
	ID              uuid.UUID `json:"id"`
	Name            string    `json:"name"`
	SignupUrl       string    `json:"signup_url"`
	CheckedUrls     []string  `json:"checked_urls"`
...
func (q *Queries) ListMerchantsFlaggedByCodeAudit(ctx context.Context, arg ListMerchantsFlaggedByCodeAuditParams) ([]ListMerchantsFlaggedByCodeAuditRow, error) {
	rows, err := q.db.Query(ctx, listMerchantsFlaggedByCodeAudit, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMerchantsFlaggedByCodeAuditRow{}
	for rows.Next() {
		var i ListMerchantsFlaggedByCodeAuditRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.SignupUrl,
