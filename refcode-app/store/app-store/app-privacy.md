...
|---|---|
| 有蒐集嗎 | 是（兩碼國家代碼，例如 `TW`） |
| 用途 | **App 功能**（目錄把在地的服務商排前面） |
| 是否連結到使用者 | **是** |
| 是否用於追蹤 | 否 |

出處：註冊頁與帳號分頁的國家下拉選單，經 `PATCH /v1/me` 送出。

> **不要勾「位置」。** Apple 的位置類別同樣指裝置的實體位置。這是使用者自選的國家，
> Info.plist 沒有任何 `NSLocation*UsageDescription`（已驗），app 不會跳定位授權。
