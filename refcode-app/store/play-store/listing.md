...
1. **應用程式內容 → 財務功能**：要選「我的應用程式未提供任何財務功能」。
   這一頁最可能是元凶。有兩個容易誤答的地方：

   - **目錄裡有銀行的推薦碼 ≠ 這支 app 提供銀行服務。** 我們只是把使用者導去
     該銀行自己的網站，帳戶與交易全部發生在對方那裡。
   - **「有訂閱收費」不是「財務功能」。** 這一題問的是有沒有提供金融**產品**
     （貸款、投資、保險、匯款、加密貨幣、銀行），不是問有沒有跟使用者收錢。
     Pro 訂閱屬於營利方式，宣告在別的地方 —— 商店資訊的「應用程式內購」與
     營利設定的訂閱 `pro`，兩者不要混在一起答。
...
|---|---|
| 應用程式名稱 | 推薦碼交流站 |
| 套件名稱 | `com.referra.app`（與 `capacitor.config.ts` 一致，**發布後永遠不能改**） |
| 應用程式類型 | 應用程式 |
