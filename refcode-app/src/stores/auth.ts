...
export const useAuthStore = defineStore('auth', () => {
  ...
  // 登入後把 RevenueCat 的身分綁到這個帳號 —— 不綁的話購買會掛在匿名 ID 上，
  // webhook 回來後端認不得，訂閱就跟著裝置而不是跟著人。
  async function syncSubscription(u: User) {
  ...
  async function restore() {
    ...
    try {
      ...
      await syncSubscription(user.value)
    } catch {
      await clearTokens()
      user.value = null
    } finally {
  ...
  async function login(email: string, password: string) {
    ...
    user.value = res.user
    await syncSubscription(res.user)
  }
